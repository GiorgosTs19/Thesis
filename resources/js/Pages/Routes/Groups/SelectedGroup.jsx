...
export const SelectedGroup = ({group, setSelectedGroup, worksPaginationInfo, setWorksPaginationInfo}) => {
    ...
    const renderAuthorItem = useCallback((item, index) => {
        return (
            <AuthorItem author={item} index={index} key={index}>
                <UtilityModal acceptText={`Remove ${item.name}`} header={'Remove member'} onAccept={() => api.groups.removeMember(group, item)} buttonClassName={styles.button}
    ...
    return <div className={'py-5 px-8 w-full'}>
        ...
        {group.parent && (
            <Badge className={styles.badge} onClick={() => setSelectedGroup(group.parent.id)}>
        ...
        <>
            ...
            {width <= 640 ?
                ...
                : <div className={'flex lg:flex-row flex-col gap-10 min-h-96'}>
                    <Card className={'w-full lg:max-w-fit '}>
                        <List data={group.members} renderFn={renderAuthorItem} wrapperClassName={`mb-auto`} vertical
