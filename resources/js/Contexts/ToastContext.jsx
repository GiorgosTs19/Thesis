...
const ToastIcons = {
    ERROR: (
        <div className={ToastIconClassName.ERROR}>
            <HiX className="h-5 w-5" />
        </div>
    ),
    SUCCESS: (
        <div className={ToastIconClassName.SUCCESS}>
            <HiCheck className="h-5 w-5" />
        </div>
    ),
    WARNING: (
        <div className={ToastIconClassName.WARNING}>
            <HiExclamation className="h-5 w-5" />
        </div>
    ),
    INFO: (
        <div className={ToastIconClassName.INFO}>
...
export const ToastProvider = ({ children }) => {
    ...
    return (
        <ToastContext.Provider value={contextValue}>
            {toastState.visible && (
                <Toast className={clsx('toast flex text-white', toastState.className)}>
                    <div className={'mr-3'}>{toastState.icon}</div>
                    <div className={'flex w-full flex-col gap-2 text-center text-sm'}>
                        {toastState.title}
                        <div className={`text-center text-base font-normal text-white`}>{toastState.message}</div>
                    </div>
